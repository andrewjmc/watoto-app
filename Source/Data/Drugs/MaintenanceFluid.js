...
const Drug = {
  ...
  calculate: (child: Child) => {
    ...
    var volume = DrugUtil.numberStep(child.weight <= 10 ? child.weight * 100 : (child.weight <= 20 ? 1000 + (child.weight - 10) * 50 : 1500 + (child.weight - 20) * 25),50,2)
	
    var ret = [
      {
        heading: 'Daily maintenance fluid',
        dose: `${volume} ml of the appropriate fluid (see below)`,
        route: 'IV, NG or oral',
        time: 24,
        volume: volume
      },
